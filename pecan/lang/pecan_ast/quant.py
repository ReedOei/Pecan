...
def extract_var_cond(var_pred):
    if type(var_pred) is Call:
        return to_name(var_pred.args[0]), var_pred
    else:
        return to_name(var_pred), None

class Forall(Predicate):
    def __init__(self, var_pred, pred):
        super().__init__()
        self.var_name, self.cond = extract_var_cond(var_pred)
        self.pred = pred

    def evaluate_node(self, prog):
        if len(prog.get_restrictions(self.var_name)) > 0:
            constraints = reduce(Conjunction, prog.get_restrictions(self.var_name))
            new_pred = Implies(constraints, self.pred)
        else:
            new_pred = self.pred
        return Complement(Exists(self.var_name, Complement(self.with_cond(new_pred)))).evaluate(prog)
    ...
    def __repr__(self):
        if self.cond is None:
            ...
            return '(∀{} {})'.format(self.var_name, Implies(self.cond, self.pred))
...
class Exists(Predicate):
    ...
    def evaluate_node(self, prog):
        ...
        return Projection(self.with_cond(new_pred).evaluate(prog), [self.var_name]).project()
    ...
    def __repr__(self):
        if self.cond is None:
            ...
            return '(∃{} {})'.format(self.var_name, Conjunction(self.cond, self.pred))
